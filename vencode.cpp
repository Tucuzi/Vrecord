		...
		int defaultsize)
{
	...
	if (space > 0) {
		...
		if ( (target_addr + space) > bs_va_endaddr) {
			...
			vpu_write(config, (void *)target_addr, room);
			vpu_write(config, (void *)bs_va_startaddr,(space - room));
		} else {
			vpu_write(config, (void *)target_addr, space);
		}
...
put_mp4header:
			...
			if (!enc->avc_vui_present_flag) {
				...
				if (pBuffer) {
					...
					if (ret == RETCODE_SUCCESS) {
						...
						vpu_write(enc->config, (void *)pBuffer, enchdr_param.size);
						free(pBuffer);
					} else {
		...
		if (cpu_is_mx6x()) {
			...
			if (enchdr_param.pParaSet) {
				...
				vpu_write(enc->config, (void *)enchdr_param.pParaSet, enchdr_param.size);
...
encoder_open(struct encode *enc)
{
	...
	encop.bitstreamFormat = enc->config->format;
